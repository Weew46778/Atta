#!/usr/bin/env bash
# ساخت APK آتا — کنسول سرور ماینکرافت. الگوبرداری از build-apk.sh (بسته‌بندی آفلاین سبک).
# ابزارها با scripts/setup-android-tools.sh آماده می‌شوند؛ کلید امضا خارج از گیت نگهداری شود.
set -euo pipefail
cd "$(dirname "$0")/.."
...
apk=Path('android/build-atta/unsigned.apk')
with zipfile.ZipFile(apk) as src, zipfile.ZipFile('android/build-atta/packed.apk','w',compression=zipfile.ZIP_DEFLATED) as dst:
    ...
    dst.write('android/build-atta/dex/classes.dex','classes.dex')
PY
"$JAVA" -jar "$APKSIGNER_JAR" sign --ks "$KEYSTORE" --ks-key-alias "$KEY_ALIAS" --ks-pass env:KEYSTORE_PASSWORD --key-pass env:KEYSTORE_PASSWORD --v1-signing-enabled true --v2-signing-enabled true --v3-signing-enabled true --v4-signing-enabled false --out "$OUT" android/build-atta/packed.apk
"$JAVA" -jar "$APKSIGNER_JAR" verify --verbose --print-certs "$OUT"
"$AAPT2" dump badging "$OUT" | head -6
sha256sum "$OUT" > releases/Atta-SHA256SUMS.txt
echo "APK ready: $OUT"
