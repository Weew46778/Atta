# -*- coding: utf-8 -*-
"""
Builds the single merged pack:
  Atta PhotoReal 512x — real-photo textures + PBR + real recorded sounds
  + Vibrant Visuals shader (atmospherics, lighting, color grading, water)
all in ONE .mcpack. Then zips it into dist/.
"""
...
MAN_MERGED = {
    "format_version": 2,
    "header": {
        "name": "§6Atta §bPhotoReal §a512x §7[All-in-One]",
        "description": "512x real-photo PBR textures + Vibrant Visuals shader + true recorded sounds — merged in one pack.",
        "uuid": U["merged_header"],
        "version": [3, 1, 0],
        "min_engine_version": [1, 21, 120],
    },
    "modules": [{
        "type": "resources",
        "description": "Atta PhotoReal 512x landscape + shader + sounds",
        "uuid": U["merged_module"],
        "version": [3, 1, 0],
    }],
    "capabilities": ["pbr"],
}
...
def main():
    ...
    # new merged identity: name + UUID + icon
    json.dump(MAN_MERGED, open(os.path.join(tex_root, "manifest.json"), "w"),
              indent=2)
    icon = os.path.join(REPO, "icons", "pack_icon_merged.png")
    if os.path.exists(icon):
        Image.open(icon).resize((256, 256), Image.LANCZOS).save(
            os.path.join(tex_root, "pack_icon.png"), optimize=True)
    save_colormaps(tex_root)
    make_panorama(tex_root)
    # shader files ride inside the SAME pack
    for path, data in [("atmospherics/atmospherics.json", ATMOSPHERICS),
                       ("lighting/global.json", LIGHTING),
                       ("color_grading/color_grading.json", COLOR_GRADING),
                       ("water/water.json", WATER)]:
        p = os.path.join(tex_root, path)
        os.makedirs(os.path.dirname(p), exist_ok=True)
        json.dump(data, open(p, "w"), indent=2)
    # retire the old split zips
    for old in ("AttaUltraReal.mcpack", "AttaVibrantShader.mcpack"):
        old_p = os.path.join(DIST, old)
        if os.path.exists(old_p):
            os.remove(old_p)
    out = os.path.join(DIST, "AttaPhotoReal512.mcpack")
    zipdir(tex_root, out)
    print(os.path.basename(out), f"{os.path.getsize(out)/1e6:.1f} MB")
