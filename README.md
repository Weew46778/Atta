# آتا — کنسول واقعی سرور ماینکرافت

اپ اندروید فارسی برای ساخت، مدیریت و بازی با سرور ماینکرافت؛ جدا از «باغ قصه‌ها» و با همان خط لولهٔ بسته‌بندی همین مخزن.

**نسخهٔ ۱٫۱٫۰ — اتصال واقعی:**
- **مدیریت واقعی سرورها روی میزبان‌ها**: کلاینت کامل API رسمی پتروداکتیل (PebbleHost، FalixNodes، GodLike، Bisect و…) — فهرست سرورهای واقعی حساب، روشن/خاموش/ری‌استارت واقعی، کنسول زندهٔ وب‌سوکت، ارسال دستور واقعی و بکاپ واقعی در پنل میزبان.
- **ساخت واقعی سرور روی وی‌پی‌اس (اترنوس یا هر وی‌پی‌اس)**: «عامل آتا» (`server/atta-agent.mjs`) یک‌بار روی وی‌پی‌اس اجرا می‌شود؛ سپس اپ هستهٔ بدراک/پیپر را دانلود و سرور واقعی می‌سازد و کامل مدیریت می‌کند.
- **اورلای بومی روی خود بازی**: سرویس اندروید با مجوز «نمایش روی برنامه‌ها» — ۹ زبانهٔ رنگی، بستن خودکار ۳۰ ثانیه، و هر دکمه یک دستور واقعی که با یک لمس به سرور واقعی می‌رسد.
- **۱۱ میزبان** در کاتالوگ اپ؛ برای اترنوس رایگان به‌دلیل ممنوعیت رسمیِ ابزار خودکار (خطر حذف سرور)، فقط بازکردن پنل خودش ارائه می‌شود.
- صدای زن فارسی همراه اپ («آوا»، ۱۰ گفتار) + کنترل صوتی با کلمهٔ بیداری «آتا».

```sh
npm test                                       # ۴۴ آزمون Node (شامل قرارداد واقعی پتروداکتیل)
npx playwright test tests/browser/atta.spec.js tests/browser/atta-real.spec.js
bash scripts/build-atta-apk.sh                 # ساخت دوبارهٔ APK
```

APK نسخهٔ ۱٫۱٫۰: `releases/Atta-Console-v1.1.0.apk` — جزئیات: [وضعیت آتا](docs/ATTA-STATUS.md). جادوگر ساخت داخل اپ برای برنامه‌ریزی/تمرین است؛ ساخت سفارش روی میزبان‌های پولی نیازمند پرداخت در خود میزبان است و مدیریت پس از آن کاملاً واقعی انجام می‌شود.
