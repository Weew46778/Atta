# آتا — کنسول سرور ماینکرافت (جدید)

اپ اندروید فارسی برای ساخت، مدیریت و بازی با سرور ماینکرافت؛ جدا از «باغ قصه‌ها» و با همان خط لولهٔ بسته‌بندی همین مخزن.

- **۱۱ میزبان سرور ماینکرافت داخل اپ** (اترنوس رایگان، وی‌پی‌اس اترنوس، پلاودوس، فالیکس‌نودز، پبل‌هاست، شاک‌بایت، ایپکس، بایسکت، گادلایک، نودکرافت، اسکالاکوب) با جادوگر ساخت قدم‌به‌قدم و استقرار خودکار شبیه‌سازی‌شده.
- **پنل مدیریت کامل در اپ**: کنسول زنده، بازیکن‌ها (اوپ/وایت‌لیست/بن)، تنظیمات دقیق `server.properties`، بکاپ، روشن/خاموش/ری‌استارت.
- **ورود به بازی**: کپی خودکار نشانی و پورت و بازکردن ماینکرافت بدراک یا لانچر پوجاو از داخل اپ.
- **اورلای داخل بازی**: کشیدن از لبهٔ چپ در حالت افقی؛ ۹ زبانهٔ رنگی نیمه‌شفاف با انیمیشن، پنل کشویی کنار زبانه، لمس آزاد بقیهٔ صفحه، بستن خودکار پس از ۳۰ ثانیه و بیش از ۱۲۰ دکمهٔ اپراتوری (آیتم، افکت، ماب، تله‌پورت، قانون جهان، مدیریت سرور و پشتیبانی).
- **صدای زن فارسی همراه اپ** («آوا»): ۱۰ گفتار ضبط‌شده برای راهنمای همهٔ صفحه‌ها، بدون وابستگی به موتور گوشی؛ کنترل صوتی با کلمهٔ بیداری «آتا».
- پیش‌نمایش وب: `npm run dev` سپس `/atta/index.html` — حالت بازی و اورلای همان‌جا قابل تمرین است.

```sh
npm test                                        # ۳۵ آزمون Node
npx playwright test tests/browser/atta.spec.js  # ۷ آزمون مرورگری آتا
bash scripts/build-atta-apk.sh                  # ساخت دوبارهٔ APK (نیازمند ابزارهای .tools)
```

APK نسخهٔ ۱٫۰٫۰: `releases/Atta-Console-v1.0.0.apk` — جزئیات و محدودیت‌ها: [وضعیت آتا](docs/ATTA-STATUS.md). ساخت واقعی روی میزبان‌ها در این نسخه شبیه‌سازی است و اورلای روی شبیه‌ساز داخل‌برنامه‌ای اجرا می‌شود؛ هر دو مورد در سند وضعیت توضیح داده شده‌اند.

---

# باغ قصه‌ها — پیش‌نمایش خانواده، نسخهٔ ۲

بازطراحی فارسی و راست‌به‌چپ با چهار تجربهٔ مرتبط: کودک ۸–۱۰ سال، خردسال ۲–۳ سال، مادر و پدر با چرخهٔ ۱۰ روز دور از خانه / ۵ روز در خانه.

- ۲۰۰ مأموریت پنج‌بخشی، ۲۰ الگوی معما، روایت‌های موقعیتی، ذخیرهٔ ادامهٔ مرحله و پاداش تلاش.
- ۱۲ جلسهٔ خردسال، ۱۵ کارگاه والدین، بازی مشترک و گزارش فعالیت فرزندان.
- انتخاب مستقل دو راوی زن تأییدشده؛ ۱۸ فایل صوتی محلی، **نه گویندگی کامل تمام مراحل**.
- ثبت صدای اختیاری و موقت روی دستگاه؛ صدا به سرور ارسال نمی‌شود.
- API حساب خانواده و همگام‌سازی دستی؛ **سرویس عمومی آنلاین هنوز مستقر نشده است**.

## اجرا و آزمون نسخهٔ ۲

Node.js 22.13+ برای API مبتنی بر SQLite لازم است.

```sh
npm ci
npm run dev       # وب روی پورت 5173
npm run api       # در ترمینال دیگر، API روی پورت 8787
npm test
npm run build
npm run test:family
```

پیکربندی مرورگر آزمون این محیط از Chromium در `/tmp/chromium` و کتابخانه‌های `.tools/chromium-libs/lib` استفاده می‌کند؛ روی محیط دیگر باید مسیر مرورگر و کتابخانه‌ها تنظیم شود.

**نتیجهٔ آخرین بررسی، ۹ سپتامبر ۲۰۲۶:** ساخت وب موفق، ۲۱ آزمون Node موفق و هر ۱۵ آزمون مرورگری نسخهٔ ۲ موفق؛ شامل عبور از هر ۲۰۰ مأموریت و ۱۰۰۰ ایستگاه، جلسات خردسال و کارگاه‌های والدین.

چهار APK آزمایشی در `releases/family-v2/` و بستهٔ آن‌ها در `releases/Bagh-Family-v2.zip` موجودند. امضا و ساختار این فایل‌ها بررسی شده، اما **نصب روی گوشی/شبیه‌ساز آزمایش نشده**. این APKها پیش از آخرین تغییرات روایت‌ها، گزارش والدین و اصلاح بررسی مبدأ میکروفن ساخته شده‌اند؛ بنابراین معادل آخرین کد وب نیستند و باید پیش از انتشار نهایی دوباره ساخته شوند. کلیدها و رمز امضا داخل Git نیستند؛ ساخت قابل ارتقا به نگه‌داری امن همان کلید و رمز نیاز دارد.

جزئیات محدودیت‌ها و کارهای باقی‌مانده: [وضعیت نسخهٔ ۲](docs/V2-STATUS.md).

---

## مستندات تاریخی نسخهٔ ۱

**مطالب زیر دربارهٔ نسخهٔ قدیمی‌اند و گزارش وضعیت فعلی نسخهٔ ۲ نیستند.**
...
## به‌روزرسانیِ در حال توسعه: انتخاب دو راوی

در پیش‌نمایش وب، از **بالای صفحه ← انتخاب صدا** یا **پروفایل ← انتخاب صدای راوی** می‌توانید صدای اول و دوم را بشنوید و برای کودک بزرگ‌تر، خردسال، مادر و پدر جداگانه انتخاب کنید. انتخاب‌ها روی همان دستگاه ذخیره می‌شوند. دکمهٔ «آشنایی صوتی با باغ قصه‌ها» در خانه از صدای انتخاب‌شدهٔ کودک بزرگ‌تر استفاده می‌کند.

هشت مقدمهٔ صوتی (چهار نقش × دو راوی) به‌صورت فایل محلی آماده‌اند. این قابلیت هنوز به معنی گویندگی تمام ۲۰۰ مرحله یا آماده‌شدن چهار اپ بازطراحی‌شده نیست. **APK نسخهٔ ۱.۰.۰ در بخش زیر قدیمی است و این انتخابگر را ندارد؛ در این تغییر، APK دوباره ساخته نشده است.**

آزمون این تغییر: ۱۳ تست Node و ۶ تست مرورگریِ منتخب موفق؛ شامل پخش واقعی MP3، تغییر صدا بدون پخش هم‌زمان، ذخیرهٔ مستقل نقش‌ها، حالت بی‌صدا و رابط موبایل.
...
npm test                    # content, narration asset and player tests
