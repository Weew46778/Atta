...
test('صفحه‌های کودک در ۴۳۰ پیکسل بدونِ سرریزِ افقی باز می‌شوند',async({page})=>{
  ...
  for(const [p,sel] of Object.entries(checks)){
    ...
    // بگذار انیمیشنِ ورودی تمام شود تا «چیدمانِ پایدار» سنجیده شود، نه فریمِ میانی
    await page.waitForFunction(()=>{const h=document.querySelector('.hero');return !h||h.getAnimations().every(a=>a.playState!=='running');},null,{timeout:10000}).catch(()=>{});
