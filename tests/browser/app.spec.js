...
test('dashboard, navigation, locks, mobile layout and all local images',async({page})=>{
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('/');await expect(page.locator('.hero')).toBeVisible();
...
test('complete first stage, reward once, resume checkpoint and unlock next',async({page})=>{
 await page.clock.install();await page.goto('/');await page.locator('[data-start="1"]').first().click();await solve(page,stages[0].activities[0],stages[0]);await page.locator('#close-modal').click();expect((await stored(page)).progress['1']).toBe(1);await page.reload();await page.locator('[data-start="1"]').first().click();
